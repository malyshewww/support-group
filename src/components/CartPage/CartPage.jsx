...
const CartPage = ({summaryData, addDataToFirebase, removeItemFromData, removeObjectFromData}) => {
            ...
            Object.keys(resData).map((el) => (
               <div className='cartpage__item'>
                  <div className="cartpage__additional">
                     <button onClick={() => removeObjectFromData(resData[el])}>Удалить из корзины</button>
                  </div>
                  <ul>
                     {
                        Object.keys(resData[el]).map((key, index) => {
                           ...
                           <li key={key+index}>
                              <h2>
                                 ...
                                 {
                                 key !== 'Тип бизнеса' ?
                                 <span onClick={() => removeItemFromData(resData[el], key)}>
                                    Удалить
                                 </span>
                                 :
                                 ""
